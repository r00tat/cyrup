 ...
 */
  ...
  if ( SIEVE AND isset($_POST['submit']) ) { 
    ...
    if ( $_POST['submit'] == 'Remove' ) {
      ...
      $errors = array();
	    ...
	    if ( !isset($_POST['members']) OR 0 == count($_POST['members']) ) {
	      ...
	      if ( count(array_filter($_POST['members'],'verify_email')) )
		      ...
		      array_push( $errors, "No valid emails" );
	    }

      # Check for days - some hardcoded values here...
      if ( empty($_POST['days']) OR intval($_POST['days']) < 1 OR intval($_POST['days']) > 30 )
        $days = 7;
      else 
      ...
      if ( empty($_POST['msg']) OR strlen(trim($_POST['msg'])) < 2 ) 
  ...
  sql_query( "SELECT * FROM cyrup_aliases WHERE enabled='1' AND domain_id='".$domain_id."'
						AND account_id='".$account_id."' ORDER BY alias" );
