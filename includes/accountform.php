 ...
 */
    ...
    if ( isset($_POST['action']) ) {
        ...
        if ( (isset( $_POST['id'])) AND (0 != intval($_POST['id'])) ) {
            sql_query( "SELECT * FROM cyrup_accounts WHERE id='".intval($_POST['id'])."'
        ...
        // Check account name
        if ( strlen($imap_account) >100 )
            array_push( $errors, "Account name contains more than 100 symbols" );
        if ( CYRUS_DELIMITER == '/') {
            if  ( !preg_match('/^([\w\-\.])+$/',trim($_POST['account'])) )
                array_push( $errors, "Account name empty or  contains illegal symbols" );
        } else {
            if  ( !preg_match('/^([\w\-])+$/',trim($_POST['account'])) )
                array_push( $errors, "Account name empty or  contains illegal symbols" );
        }
        ...
        if ( empty($_POST['password_']) AND ((!isset($account_id)) OR (SHOW_PASSWORD)) ) {
            ...
            if ( ($_POST['password_'] != $_POST['password_retype']) AND (!SHOW_PASSWORD) )
	...
	if ( isset( $account_id ) )
	    $query .= " AND id<>'".$account_id."'";
	if ( sql_fetch_array( sql_query( $query ) ) )
	    array_push( $errors, "Account with this name already exists" );
	if ( (!isset( $account_id)) AND ($domain_row['accounts_max'])
        AND ($domain_row['accounts_cur']>=$domain_row['accounts_max']) )
            array_push( $errors, "Maximum number of accounts for the domain reached" );

	// Check quotas
	if ( ( ! ALLOW_NO_QUOTA ) AND ( ! $quota ) ) 
	    array_push( $errors, "Zero quota not allowed" );
	if ( ($domain_row['quota']) AND  (($domain_row['quota_cur']+$quota_inc) > $domain_row['quota']) )
	    array_push( $errors, "Domain quota reached ("
                    .$domain_row['quota_cur']."+".$quota_inc." > ".$domain_row['quota'].")" );
	if ( $quota > 1000000000 )
	    array_push( $errors, "Quota exceed maximum of 1Tb" );
	    ...
	    $sel = " cyrup_accounts SET account='".$imap_account."'"
            ...
            if ( isset($account_id) ) {
                ...
                if ( $account_row['account'] != $imap_account ) {
                    sql_query( "UPDATE cyrup_aliases SET aliased_to='".$imap_account
                ...
                if ( isset($_POST['autoalias']) ) {
                    ...
                    if ( ($domain_row['aliases_max'])
                    AND ( $domain_row['aliases_max'] <= $domain_row['aliases_cur']) )
                        $cannot_create_aliases = 1;
    ...
    if ( (isset( $_GET['id'])) AND (0 != intval($_GET['id'])) ) {
        sql_query( "SELECT * FROM cyrup_accounts WHERE id='".intval($_GET['id'])."'
				    AND domain_id='".$domain_id."'"  );
        if ( 1 == sql_num_rows() ) {
