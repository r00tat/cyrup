<?php
/*
 * $RCSfile: imap.inc.php,v $ $Revision: 1.12 $
 ...
 */
	...
	$folders = array_map( 'trim', explode(',',IMAP_FOLDERS) );
    ...
    function cimap_getquota( $mbox_name ) {
        ...
        if ( ! cimap_command( "getquota user".CYRUS_DELIMITER.$mbox_name ) )
            return $rval;

        if ( ! preg_match( "/STORAGE (\d+) (\d+)/", $_imap_result[0], $quota ) )
            return $rval;
